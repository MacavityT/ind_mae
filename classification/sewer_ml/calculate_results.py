...
def calcualteResults(args):
    ...
    for subdir, dirs, files in os.walk(scorePath):
        ...
        for scoreFile in files:
            ...
            scoresDf = scoresDf.sort_values(by=["Filename"]).reset_index(
                drop=True)
            ...
            if split.lower() == "test":
                outputName = outputName[:len(outputName) -
                                        len("_test_sigmoid.csv")]
            elif split.lower() == "val":
                outputName = outputName[:len(outputName) -
                                        len("_val_sigmoid.csv")]
            elif split.lower() == "train":
                outputName = outputName[:len(outputName) -
                                        len("_train_sigmoid.csv")]

            with open(os.path.join(outputPath, '{}.json'.format(outputName)),
                      'w') as fp:
                json.dump(
                    {
                        "Labels": Labels,
            ...
            newString = "{:.2f} & {:.2f} ".format(
                new["F2"] * 100, auxillary["F1_class"][-1] * 100)

            aveargeString = "{:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f}".format(
                main["mF1"] * 100, main["MF1"] * 100, main["OF1"] * 100,
                main["OP"] * 100, main["OR"] * 100, main["CF1"] * 100,
                main["CP"] * 100, main["CR"] * 100, main["EMAcc"] * 100,
                main["mAP"] * 100)

            classF1String = " & ".join(
                ["{:.2f}".format(x * 100) for x in auxillary["F1_class"]])
            classF2String = " & ".join(
                ["{:.2f}".format(x * 100) for x in new["F2_class"]])
            classPString = " & ".join(
                ["{:.2f}".format(x * 100) for x in auxillary["P_class"]])
            classRString = " & ".join(
                ["{:.2f}".format(x * 100) for x in auxillary["R_class"]])
            classAPString = " & ".join(
                ["{:.2f}".format(x * 100) for x in auxillary["AP"]])

            with open(
                    os.path.join(outputPath,
                                 '{}_latex.txt'.format(outputName)),
                    "w") as text_file:
